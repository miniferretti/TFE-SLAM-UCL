...
class ProcessScan:
    ...
    def __init__(self, ranges, display_output_flag):

        self.nrows = ranges.shape[0]
        ranges = np.transpose(ranges)

        self.ranges_data = pd.DataFrame(ranges, columns={"R", "T"})
        self.ranges_data = self.ranges_data[0:self.nrows]

        self.kdtree_main = KdTree_class()
        self.kdtree_root_node = self.kdtree_main.insert_points(
            self.ranges_data, display_output=display_output_flag)
    ...
    def euclidean_clustering(self, distance_threshold, cluster_parameters):
        ...
        for point in self.pcd_data.iterrows():
            ...
            if not processed_flag[index]:
                base_cluster = set()
                self.find_clusters(current_point, base_cluster, index,
                                   distance_threshold, cluster_parameters, processed_flag)
...
if __name__ == "__main__":
    ...
    APPLICATION = ProcessPointCloud(
        pcd_file="point_cloud_data_sample_2.pcd", nrows_value=6000, display_output_flag=False)
    clusters = APPLICATION.euclidean_clustering(
        distance_threshold=5, cluster_parameters={"min_size": 2}) 
    # print(clusters)
